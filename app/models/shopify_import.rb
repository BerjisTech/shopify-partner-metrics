...
class ShopifyImport < ApplicationRecord
  ...
  class << self
    ...
    def run_data(app_id, api, time, data_set, cursor = '')
      ...
      return Rollbar.error(data) unless data.present?

      data = JSON.parse(data)

      records = OpenStruct.new data['data']

      return Rollbar.error(records) unless records.present?
    ...
    def process(app_id, api, time, records, data_set, cursor)
      ...
      return Rollbar.error(records) unless records['app'].present? || records['transactions'].present?
      ...
      return Rollbar.error(results) unless results.present? && results.size.positive?
